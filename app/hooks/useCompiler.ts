...
export function useCompiler(initialCode: string) {
  ...
  const [structDefs, setStructDefs] = useState<Record<string, {name: string, type: string}[]>>({});
  ...
  const logMessage = useCallback((msg: string, type: string = 'system') => {
    setHistory(prev => [...prev, msg].slice(-200));
  }, []);
  ...
  const resetCompiler = useCallback(() => {
    ...
    const localStructDefs: Record<string, {name: string, type: string}[]|any> = {};
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('struct') && line.includes('{')) {
          const nameMatch = line.match(/struct\s+(\w+)\s*\{/);
          if (nameMatch) {
            ...
            while (j < lines.length && !lines[j].includes('}')) {
              ...
              const fieldMatch = fieldLine.match(/^(int|char|float|double|struct\s+\w+)\s+(\w+);/);
              if (fieldMatch) {
                fields.push({ type: fieldMatch[1], name: fieldMatch[2] });
              }
              j++;
            }
            localStructDefs[structName] = fields;
            i = j; // skip the struct body
          }
          continue;
        }
  ...
  const updateOrAddVariable = useCallback((name: string, value: string, type: 'value' | 'pointer' | 'struct' | 'array' = 'value') => {
    const normalizedNameString = name.replace('->', '.');
    // If it's a deep access like struct.field = val
    const nameParts = normalizedNameString.split('.');
    const rootName = nameParts[0];
    const tailParts = nameParts.slice(1);
    ...
    if (isPtrAccess) {
      ...
      if (name.startsWith('*')) {
        // Multi-level dereference: **k = 5
        targetAddr = evaluateExpression(name.substring(1));
      } else if (name.includes('->')) {
        // Pointer access: ptr->field = 10
        const root = name.split('->')[0];
        subKey = '.' + name.split('->').slice(1).join('.');
        targetAddr = evaluateExpression(root);
      }
    ...
    if (stack.length === 0 || !stack[stack.length-1].variables.find(v => v.name === rootName)) {
       const globalIdx = globals.findIndex(g => g.name === rootName);
       if (globalIdx !== -1) {
          setGlobals(prev => {
            const next = [...prev];
            if (tailParts.length === 0) {
              ...
              // Update nested property in a flattened struct string like {.fan.isOn: 0, .fan.power: 0}
              let curVal = next[globalIdx].value;
              const targetPath = tailParts.join('.');
              const keySearch = `.${targetPath}:`;
              
              if (curVal.includes(keySearch)) {
                const escapedPath = targetPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                // Support optional spaces and different value formats
                const regex = new RegExp(`\\.${escapedPath}:\\s*[^,}]+`);
                curVal = curVal.replace(regex, `.${targetPath}: ${value}`);
              } else {
                if (curVal === '0' || curVal === 'NULL' || !curVal.includes('{')) {
                  curVal = `{.${targetPath}: ${value}}`;
                } else {
                  curVal = curVal.replace('}', `, .${targetPath}: ${value}}`);
                }
              }
              next[globalIdx] = { ...next[globalIdx], value: curVal };
            }
            return next;
          });
          return;
    ...
    setStack(prev => {
      ...
      for (let i = updated.length - 1; i >= 0; i--) {
        const idx = updated[i].variables.findIndex(v => v.name === rootName);
        if (idx !== -1) {
          targetFrameIdx = i;
          varIndex = idx;
          break;
      ...
      if (tailParts.length === 0) {
        ...
        // Update nested property in a flattened struct string like {.fan.isOn: 0, .fan.power: 0}
        let curVal = newVars[varIndex].value;
        const targetPath = tailParts.join('.');
        const keySearch = `.${targetPath}:`;
        ...
        newVars[varIndex] = { ...newVars[varIndex], value: curVal };
      }
        ...
        const size = type === 'pointer' ? 8 : (type === 'struct' ? 32 : type === 'array' ? 40 : 4);
  ...
  const evaluateExpression = useCallback((expr: string): string => {
    ...
    while (iterations < 100) {
      ...
      if (v) {
        ...
        const boundaryRegex = new RegExp(`(^|[^a-zA-Z0-9_])${escaped}(?![a-zA-Z0-9_])`, 'g');
        finalExpr = finalExpr.replace(boundaryRegex, (m, p1) => (p1 || '') + resolvedValue);
      } else {
        if (/^[a-zA-Z_]\w*$/.test(fullMatch)) {
    ...
    // Convert hex to decimal before eval for arithmetic safety
    const isPureAddress = finalExpr.trim().match(/^0x[0-9A-Fa-f]+$/);
    if (!isPureAddress) {
      finalExpr = finalExpr.replace(/0x[0-9A-Fa-f]+/g, (m) => {
        try { return parseInt(m, 16).toString(); } catch { return '0'; }
      });
    }

    try {
        if (isPureAddress) return isPureAddress[0].trim();
        // Handle potential issues with scientific notation or weird characters from eval
        ...
        // If eval fails, it might be a literal string or an address that escaped regex
  ...
  const stepCode = useCallback(() => {
    ...
    const remainingInLine = statement.startsWith('for') ? "" : statement.split(';').slice(1).join(';').trim();
      ...
      if (!line || line.startsWith('#')) { 
        if (remainingInLine) {
          // If there's more on this line, we don't advance the physical line yet
          // Actually, this is complex with 'swallowed'. 
          // For now, simpler: we don't support multi-statements well, but we should at least advance.
        }
        setCurrentLine(prev => prev + swallowed); 
        return; 
      }
        ...
        if (line.startsWith('printf')) {
          ...
          if (match) {
            ...
            let output = format.replace(/%d|%u|%p|%f|%s/g, (m) => {
              ...
              if ((m === '%d' || m === '%u') && valStr.startsWith('0x')) {
                return parseInt(valStr, 16).toString();
              }
              return valStr;
            });
      ...
      } else if (line.startsWith('int ') || line.startsWith('char ') || line.startsWith('float ') || line.startsWith('double ') || (line.startsWith('struct ') && !line.includes('{'))) {
        ...
        if (typeMatch) {
          ...
          rest.split(',').forEach(part => {
             ...
             if (vMatch) {
                ...
                const isPointer = nameRest.includes('*');
                const name = nameRest.replace(/\*/g, '').trim();
                
                let initialValue = '0';
                if (initial) {
                  initialValue = evaluateExpression(initial);
                } else if (type.startsWith('struct')) {
                  const structName = type.replace('struct', '').trim();
                  const getFlattenedInit = (sName: string, prefix: string = ""): string => {
                    const fields = structDefs[sName] || [];
                    let resFields: string[] = [];
                    fields.forEach(f => {
                      const fullKey = prefix + "." + f.name;
                      if (f.type.startsWith('struct')) {
                        const nested = getFlattenedInit(f.type.replace('struct', '').trim(), fullKey);
                        resFields.push(nested.substring(1, nested.length - 1));
                      } else {
                        resFields.push(`${fullKey}: 0`);
                      }
                    });
                    return "{" + resFields.join(', ') + "}";
                  };
                  initialValue = getFlattenedInit(structName);
                }

                if (arrSize) {
                   initialValue = '{';
                   const size = parseInt(arrSize);
                   for (let k = 0; k < size; k++) initialValue += `[${k}]: 0${k < size - 1 ? ', ' : ''}`;
                   initialValue += '}';
