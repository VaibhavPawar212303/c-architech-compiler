...
export default function InteractiveStack({ 
  ...
  const [manualStack, setManualStack] = useState<StackItem[]>([]);
  ...
  // Derived stack from system props if available
  const displayStack = (systemStack && systemStack.length > 0) 
    ? systemStack.flatMap((frame, fIdx) => 
        (frame.variables || []).map((v: any, vIdx: number) => ({
          id: `sys-${fIdx}-${vIdx}`,
          value: `${v.name}${v.type ? `: ${v.type}` : ''}`,
          actualValue: v.value,
          color: frame.name === 'main' ? 'bg-blue-600' : 'bg-indigo-500',
          isSystem: true,
          frameName: frame.name,
          isTop: (fIdx === systemStack.length - 1) && (vIdx === frame.variables.length - 1)
        }))
      ).reverse() // Show top of stack at top
    : manualStack;
  ...
  useEffect(() => {
    ...
    const initialLogs: LogEntry[] = [
      ...
      { id: '2', timestamp: new Date().toLocaleTimeString(), message: 'KERNEL_READY: TRUE', type: 'system' }
    ];
    setLogs(initialLogs);
  }, []);

  // Log changes in system stack
  const prevStackLen = useRef(0);
  const prevSystemStack = useRef<any[]>([]);
  const [pulsePop, setPulsePop] = useState(false);
  const [poppedInfo, setPoppedInfo] = useState<string | null>(null);

  useEffect(() => {
    if (systemStack) {
      if (systemStack.length !== prevStackLen.current) {
        if (systemStack.length > prevStackLen.current) {
          const activeFrame = systemStack[systemStack.length - 1];
          addLog(`STACK_PUSH: frame "${activeFrame.functionName}" added`, 'system');
        } else if (prevSystemStack.current.length > 0) {
          const removedFrame = prevSystemStack.current[prevSystemStack.current.length - 1];
          const info = `frame "${removedFrame.functionName}"`;
          addLog(`STACK_POP: context restored from ${info}`, 'result');
          setPoppedInfo(info);
          setPulsePop(true);
          setTimeout(() => {
            setPulsePop(false);
        ...
        prevStackLen.current = systemStack.length;
      }
      prevSystemStack.current = [...systemStack];
    } else if (prevStackLen.current > 0) {
      addLog(`STACK_RESET: idle`, 'system');
      prevStackLen.current = 0;
      prevSystemStack.current = [];
    }
  }, [systemStack]);
  ...
  const push = () => {
    ...
    const newItem: StackItem = {
      id: Math.random().toString(36).substr(2, 9),
      value: inputValue,
      color: COLORS[manualStack.length % COLORS.length],
    };
    setManualStack(prev => [newItem, ...prev]);
    setInputValue('');
  ...
  const pop = () => {
    ...
    const popped = manualStack[0];
    setManualStack(prev => prev.slice(1));
    const actionMsg = `POP() -> "${popped.value}"`;
    setLastAction(actionMsg);
    addLog(actionMsg);
    setPeeked(false);
    
    // Visual feedback for manual pop
    setPoppedInfo(`"${popped.value}"`);
    setPulsePop(true);
    setTimeout(() => {
      setPulsePop(false);
      setPoppedInfo(null);
  ...
  const peek = () => {
    if (manualStack.length === 0) return;
    setPeeked(true);
    const actionMsg = `PEEK() -> "${manualStack[0].value}"`;
    setLastAction(actionMsg);
    ...
    <div className={`flex flex-col h-full w-full ${bgColor} ${textColor} overflow-hidden font-sans`}>
      ...
      <div className={`p-6 border-b ${borderColor} flex items-center justify-between backdrop-blur-md z-10 ${theme === 'dark' ? 'bg-black/40' : 'bg-white/40'}`}>
        ...
        <div className="flex items-center gap-6">
          <div className="flex flex-col items-end">
            ...
            <span className="text-2xl font-mono font-bold">{displayStack.length}</span>
          </div>
          <div className="h-10 w-px bg-white/10" />
          <div className="flex flex-col items-end">
            <span className="text-[10px] font-black uppercase tracking-widest opacity-30">Status</span>
            <span className={`text-xs font-black uppercase tracking-widest ${displayStack.length === 0 ? 'text-amber-500' : 'text-emerald-500'}`}>
              {systemStack && systemStack.length > 0 ? 'Live_Sync' : (displayStack.length === 0 ? 'Empty' : 'Active')}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 relative flex flex-col items-center justify-end p-12 overflow-hidden bg-grid-white/[0.02]">
            ...
            {pulsePop && (
              ...
              >
                <div className="flex flex-col items-center">
                  <div className="bg-red-500 text-white px-6 py-3 rounded-full font-black uppercase tracking-widest shadow-2xl flex items-center gap-3">
                    <ArrowUp size={24} /> 
                    <span>POP_EVENT</span>
                  </div>
                  <span className="text-red-400 font-mono text-[10px] mt-2 whitespace-nowrap bg-black/40 px-2 py-1 rounded border border-red-500/20">{poppedInfo}</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className={`relative w-80 border-x-4 border-b-4 ${borderColor} ${theme === 'dark' ? 'bg-white/[0.02]' : 'bg-black/[0.02]'} min-h-[500px] flex flex-col justify-end p-4 gap-2 transition-all`}>
            ...
            <AnimatePresence initial={false}>
              {displayStack.map((item: any, index) => (
                <motion.div
                  key={item.id}
                  ...
                  animate={{ 
                    ...
                    opacity: 1, 
                    scale: 1,
                    boxShadow: index === 0 && peeked ? '0 0 30px rgba(59, 130, 246, 0.6)' : 'none',
                    zIndex: displayStack.length - index
                  }}
                  exit={{ y: -100, x: 50, opacity: 0, rotate: 5, scale: 0.9 }}
                  transition={{ type: 'spring', damping: 25, stiffness: 400 }}
                  className={`w-full ${item.actualValue && String(item.actualValue).length > 50 ? 'h-32' : 'h-20'} ${item.color} border-2 border-white/20 flex flex-col justify-center px-6 relative group cursor-pointer hover:brightness-110 transition-all`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <span className="text-[10px] font-black opacity-30">[{displayStack.length - 1 - index}]</span>
                      <span className="text-sm font-black uppercase tracking-tight text-white">{item.value}</span>
                    </div>
                    {item.frameName && (
                      <span className="text-[8px] font-mono bg-black/40 px-1.5 py-0.5 rounded text-white/70 border border-white/10">{item.frameName}</span>
                    )}
                  </div>
                  
                  <div className={`mt-2 font-mono text-[10px] text-white/90 truncate max-w-full ${item.actualValue !== undefined && String(item.actualValue).length > 50 ? 'whitespace-normal line-clamp-3' : ''}`}>
                    {item.actualValue !== undefined ? String(item.actualValue) : <span className="opacity-20 italic">NULL_REF</span>}
                  </div>
        ...
        <aside className={`w-[400px] border-l ${borderColor} ${cardBg} p-8 flex flex-col gap-8`}>
          {/* Operation Input */}
          <div className={`space-y-4 ${systemStack && systemStack.length > 0 ? 'opacity-50 pointer-events-none' : ''}`}>
             <div className="flex items-center justify-between">
                <h2 className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2 opacity-50">
                  <Activity size={14} /> {systemStack && systemStack.length > 0 ? 'Kernel_Managed' : 'System_Operations'}
                </h2>
                {systemStack && systemStack.length > 0 && (
                  <span className="text-[8px] font-mono bg-blue-500/20 text-blue-500 px-2 py-0.5 rounded">AUTO</span>
                )}
             </div>
