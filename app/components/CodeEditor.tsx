...
const CodeEditor = ({
  ...
  useEffect(() => {
    if (currentLine >= 0 && textareaRef.current) {
      const lineHeight = 24;
      ...
      if (targetScroll < currentScroll || targetScroll > (currentScroll + containerHeight - 80)) {
        textareaRef.current.scrollTo({
          top: Math.max(0, targetScroll - 100),
          behavior: 'smooth'
        });
      }
    }
      ...
      <div className="flex-1 relative font-mono text-[14px] overflow-hidden flex">
        ...
        <div className="flex-1 relative overflow-hidden">
          ...
          <textarea
            ...
            className={`absolute inset-0 w-full h-full bg-transparent p-6 pl-6 resize-none focus:outline-none custom-scrollbar leading-[24px] z-10 selection:bg-blue-500/30 font-mono caret-blue-500 whitespace-pre overflow-x-auto ${
          ...
          <pre
            ref={preRef}
            aria-hidden="true"
            className="absolute inset-0 p-6 pl-6 pointer-events-none whitespace-pre overflow-hidden leading-[24px] z-0"
