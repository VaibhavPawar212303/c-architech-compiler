...
const StackVisualizer = ({ stack, theme = 'dark' }: StackVisualizerProps) => {
  return (
    <div className="relative flex flex-col h-full">
      ...
      <div className="flex-1 p-3 md:p-6 flex flex-col gap-3 md:gap-4 relative z-10 overflow-y-auto overflow-x-auto custom-scrollbar min-h-0">
              ...
              >
                ...
                <div className={`space-y-1 pt-4 border-t ${theme === 'dark' ? 'border-white/5' : 'border-black/5'}`}>
                  {frame.variables.map((v, i) => (
                    ...
                    >
                      <div 
                        className={`flex items-center gap-2 md:gap-4 p-2 md:p-3 rounded-lg transition-all ${
                          theme === 'dark' ? 'hover:bg-white/[0.02]' : 'hover:bg-slate-50'
                        }`}
                      >
                        {/* Var Identification */}
                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 shrink-0 sm:min-w-[160px]">
                          <div className="w-[50px] md:w-[80px] shrink-0">
                            <code className={`text-[8px] md:text-[9px] font-mono block ${theme === 'dark' ? 'text-blue-400/60' : 'text-blue-600/60'}`}>
                              {v.address.toUpperCase()}
                            </code>
                          </div>

                          <div className="min-w-[50px] md:min-w-[80px] shrink-0">
                            <span className={`text-[10px] font-bold truncate block ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
                              {v.name}
                            </span>
                          </div>
                        </div>
                        ...
                        {/* Value display */}
                        <div 
                          className={`flex-1 flex flex-col md:flex-row md:items-center md:justify-between px-2 md:px-3 py-1 md:py-1.5 rounded-md relative min-h-[32px] ${
...
const renderHierarchy = (valueStr: string, baseAddr: string, theme: 'dark' | 'light' = 'dark') => {
  ...
  const renderRecursive = (node: TreeNode, depth: number = 0) => {
    const sortedChildren = Array.from(node.children.values()).sort((a, b) => {
      // Sort indices numerically, fields alphabetically
      if (a.isIndex && b.isIndex) {
        return parseInt(a.label.slice(1, -1)) - parseInt(b.label.slice(1, -1));
      }
      return a.label.localeCompare(b.label);
    });

    return sortedChildren.map((child, cIdx) => (
      <div key={child.key + cIdx} className="space-y-1">
        <div 
          className={`flex items-center justify-between group/line py-1 border-l transition-all duration-300 ${
            theme === 'dark' 
              ? 'border-white/5 hover:border-blue-500/50 hover:bg-blue-500/5 shadow-sm' 
              : 'border-black/5 hover:border-blue-600 hover:bg-blue-50/50 shadow-sm'
          }`}
          style={{ 
        ...
        >
          <div className="flex items-center gap-1.5 overflow-hidden flex-1">
            <span className="text-[7px] font-mono opacity-20 tracking-tighter tabular-nums whitespace-nowrap min-w-[30px] hidden sm:inline">
              0X{child.address.substring(2).toUpperCase()}
            </span>
            <div className="flex items-center gap-1 overflow-hidden">
               {depth > 0 && <span className="opacity-10 text-[9px] flex-shrink-0">└─</span>}
               <span className={`text-[9px] md:text-[10px] font-mono truncate transition-colors ${
                 child.isIndex 
                   ? (theme === 'dark' ? 'text-slate-500' : 'text-slate-400') 
                   : (theme === 'dark' ? 'text-blue-400 group-hover/line:text-blue-300' : 'text-blue-600 font-bold')
               }`}>
          ...
          {child.value !== undefined && (
            <div className="flex items-center gap-2 ml-2">
              <span className="text-[8px] opacity-20 font-mono hidden md:inline">:</span>
              <motion.span 
                key={child.value}
                initial={{ scale: 1.1, color: '#10b981' }}
                animate={{ scale: 1, color: theme === 'dark' ? '#34d399cc' : '#059669' }}
                className={`text-[10px] font-black border-b border-dashed tabular-nums px-1 rounded transition-colors whitespace-nowrap ${
                theme === 'dark' ? 'border-emerald-500/20' : 'border-emerald-600/30'
              }`}>
                {child.value}
              </motion.span>
            </div>
  ...
  return <div className="space-y-0.5 overflow-x-auto custom-scrollbar pb-1">{renderRecursive(root)}</div>;
};
